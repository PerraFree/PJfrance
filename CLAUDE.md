...
| `src/lib/photos.ts` + `src/components/PhotoForm.tsx` | Community-foton ("📷 Lägg till foto", på befintliga OCH nya platser). Skalas ner klientsidan (max 1280px, JPEG q0.72, gräns 6 MB på originalfilen) innan uppladdning till Supabase Storage (`place-photos`-bucket, samma 6 MB-gräns satt på bucketen). Hålls medvetet litet för att inte äta av Supabase gratisnivåns lagring. **Publiceras direkt, INGEN granskningskö** (beslut aug 2026 – Per hinner inte bevaka ännu en kö). Olämpligt innehåll flaggas i stället via befintliga "Rapportera fel" |
